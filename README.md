...
为阅读、学习或工作建立每天重复的时长目标，随时开始、暂停或切换计划。每个计划可以使用独立的番茄钟节奏；界面同时显示当前番茄轮次和当天累计进度，收起面板或切换页面不会停止计时。
...
在 Plans 页面点击 **Add Daily Plan** 可以设置目标时长、启用日期和番茄钟节奏。点击计划右侧的播放按钮开始计时；Today 显示所有计划与当天进度，Focus 提供只保留当前倒计时的专注界面。专注阶段计入每日目标，休息阶段不计入。
